...
前回は、class宣言の方法について整理しました。

@[card](https://zenn.dev/pometa/articles/7f70cc2fae9cb3)

rubicon-objcの方が、Pythonicなclass宣言でした。つまり、objc_utilの`create_objc_class`に依存した書き方ができなくなります。

今回は、protocol(delegate)を中心に、コードの書き換えの整理をします。
...
Apple Developer Documentationで`Protocol`と表記されている場合に使用します。
`UITableViewDataSource`のように`Delegate`が名前に入っていない場合もあります。まずは、(Apple)公式Documentationで調べることが大切です。

### objc_util の場合

classの宣言と同様に、[create_objc_class](https://omz-software.com/pythonista/docs-3.4/py3/ios/objc_util.html#objc_util.create_objc_class)で宣言をしていました。

@[card](https://omz-software.com/pythonista/docs-3.4/py3/ios/objc_util.html#objc_util.create_objc_class)

以下のコードは、閉じるボタンを右肩に持たせた[`UINavigationController`](https://developer.apple.com/documentation/uikit/uinavigationcontroller?language=objc)を実装した例(一部中略)です。
`ui`モジュールの[ui.NavigationView](https://omz-software.com/pythonista/docs-3.4/py3/ios/ui.html#ui.NavigationView)のようなものをイメージしています(`ui`モジュールを使わずに、Pythonista3上で独自にUIKitを呼び出す場面で使用)。

このコードで、プロトコル定義とデリゲートメソッドを呼び出しています。
...
blockのことは、audioでやるので、delegateするために、protocolを呼び出す感じで説明をしていく。
