...
実行後、裏側の console に Life Cycle ごとに`print` されているのが確認できます。
...
1 ファイルで（約 400 行程ありますが）a-Shell を使い View の開閉ができました。全体の処理の流れをざっくり掴んでもらえていたら嬉しいです。

mac を使い Xcode で開発をすると、ビルドなど時間を取られることが多くあります。その点 a-Shell は、フットワーク軽くトライアンドエラーができる面白さがあると感じます。
