---
title: "【非推奨】Rubicon-ObjCの手動インストール・インポート"
emoji: "🈲"
...
- iOS 端末上では、1 リポジトリ（プロジェクトフォルダ）運用
...
（私の場合は、WorkingCopy で clone して取得しています）
...
（このバージョンのおかげで、気軽に Pythonista3 で開発ができる）
...
PC 環境では、決して好ましいとはいえない、インストール・インポート・環境設定ですので、冒頭で提示した内容を再度掲示して終わりにします。
