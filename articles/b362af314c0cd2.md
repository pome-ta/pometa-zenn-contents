...
[a-Shell](https://holzschu.github.io/a-Shell_iOS/) で、[SF Symbols](https://developer.apple.com/jp/sf-symbols/) の一覧を出す簡単なアプリを実装します。
...
今まで紹介したライブラリ、パッケージを使い、端末内にある SF Symbols データの取り出しから、表示までをハンズオン形式で実装していきます。
...
空の新規ファイルから、順を追って実装していきます。最終的には`exampleSFSymbolsViewer.py` のコードになります。


先に、完成予定のコードです。


```python: exampleSFSymbolsViewer.py
