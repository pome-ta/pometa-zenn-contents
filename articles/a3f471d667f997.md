...
- 【制作編】#3: a-Shell(mini) の Python で UIKit から View を呼び出す【Rubicon-ObjC】
  ...
  - Swift, Objective-C のコートドを Rubicon-ObjC へ落とし込み
...
しかし、Apple Developer Documentation でも Enumeration Case は（Swift も Objective-C のページも）値が不明なことが多いです。
...
私が`asyncio` にわか勢なので、`logging` module で状態を確認しようしているのがわかりますね。
...
Rubicon-ObjC で実装するにあたり、解説記事ブログも参考になりますが、[Apple Developer Documentation](https://developer.apple.com/documentation) で調べないと必要情報を得ることは難しい場面が多いです。
