...
### a-Shell はファイルアプリ経由の参照が可能です

a-Shell でコード編集が大変、または既に実行したいデータがある場合は、アプリ外ディレクトリにアクセスするコマンド`pickFolder` や、Git (導入方法は割愛)を使った開発もできます。
...
演算が必要（に、なりそう）な要素は、`IntFlag` で定義しています。
