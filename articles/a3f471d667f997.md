...
`__init__.py` は、空ファイルである事が多いですが、2点設定しています。コード全体に影響させます。
...
Main Thread 外でのUI 更新や、配列のindex 外の呼び出しなどを教えてくれます。捕捉してくれるエラーは少ないですが、デバッグする手立てが少ないRubicon-ObjC では小さな情報でもありがたいのです。
...
Enumeration Case として定義されているまとまりです。class で要素をまとめたいので、Swift の表記で統一しています。
演算が必要になる（なりそう）な要素は、`@dataclass` しています。
定義する値に関して、Apple Developer Documentation でEnumeration Case は（Swift もObjective-C のページも）不明なことが多いです。

@[card](https://developer.apple.com/documentation/uikit/uimodalpresentationstyle)


その場合は、Rust のApple frameworks バインディング [`objc2`](https://github.com/madsmtm/objc2) からsource を確認しています。
...
`Function` を定義まとめて格納する場所です。現在は`NSStringFromClass` 1つのみです。


`NSStringFromClass` はClass 名を文字列で返します。必須ではありませんが、今回はView のLife Cycle 確認のため`print` する目的で使用しています。
...
### `lifecycle=iOSLifecycle()` ではない


event loop の変数`loop` は、この後に説明をする[class `App`](#`App.py`) の中でevent loop を走らせることになります。


Rubicon-ObjC のドキュメント[Asynchronous Programming with Rubicon - Rubicon 0.5.0](https://rubicon-objc.readthedocs.io/en/stable/how-to/async.html) にて、`asyncio` とevent loop の解説があります。


iOS [Integrating asyncio with iOS and UIApplication | Asynchronous Programming with Rubicon - Rubicon 0.5.0](https://rubicon-objc.readthedocs.io/en/stable/how-to/async.html#integrating-asyncio-with-ios-and-uiapplication)
...
しかし`get_event_loop` は、`logging` で`Warning` が出るので`new_event_loop` としています。どっちが正解かはまだ整理できていません。
