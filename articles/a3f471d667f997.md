...
今回のView の表示結果は、前回と変わりありません。1 ファイル（`singleFileSample.py`）だったコードをmodule 化し、煩雑さの回避、取り回しをよくするのが目的です。
...
しかし、Apple Developer Documentation でもEnumeration Case は（Swift もObjective-C のページも）値が不明なことが多いです。

@[card](https://developer.apple.com/documentation/uikit/uimodalpresentationstyle)


その場合は、Rust のApple frameworks バインディンクの゙ [`objc2`](https://github.com/madsmtm/objc2) からsource を確認しに行っています。
...
## おまけの補足: Rubicon-ObjC でのFunction 定義方法
...
[Foundation](https://developer.apple.com/documentation/foundation?language=objc) はFramework です。幸いにもRubicon-ObjC では[`rubicon.objc.runtime.Foundation`](https://rubicon-objc.readthedocs.io/en/stable/reference/rubicon-objc-runtime.html#rubicon.objc.runtime.Foundation) で定義されているので、`import` できます。

```python
from pyrubicon.objc.runtime import Foundation
```

他にRubicon-ObjC で、`libc`, `libobjc` は定義されています:
...
それ以外のFramework は、[`load_library`](https://rubicon-objc.readthedocs.io/en/stable/reference/rubicon-objc-runtime.html#rubicon.objc.runtime.load_library) を使い、自身でload する必要があります。
