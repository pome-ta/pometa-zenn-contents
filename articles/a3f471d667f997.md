...
今回の View の表示結果は、前回と変わりありません。1 ファイル（`singleFileSample.py`）だったコードを module 化し、煩雑さの回避、取り回しをよくするのが目的です。
...
`rbedge` ディレクトリが、module の機能を持ちます。`singleFileSample.py` を分割したコードです。
...
として、`rbedge` と命名してみました。別の module 名でも問題はありません。
...
`__init__.py` は、空のファイルである事が多いですが、コード全体に影響させたいものとして、以下 2 点設定しています。

### `ObjCClass.auto_rename` の`True`

連続で実行する際の class name 衝突エラーを回避します。
...
しかし、Apple Developer Documentation でも Enumeration Case は（Swift も Objective-C のページも）値が不明なことが多いです。

@[card](https://developer.apple.com/documentation/uikit/uimodalpresentationstyle)

その場合は、Rust の Apple frameworks バインディンクの [`objc2`](https://github.com/madsmtm/objc2) から source を確認しに行っています。
...
`NSStringFromClass` は引数にした Class 名を文字列で返します。必須ではありませんが、今回は View の Life Cycle 確認のため`print` する目的で使用しています。
...
非同期の event loop が走ることで、a-Shell でクラッシュせずに View を表示することができます。
...
Rubicon-ObjC ドキュメントの iOS は、「そのアプリ単体（つまり、通常のアプリ挙動）」で、起動・終了を管理しますが、起動・終了は a-Shell 側が持っていることになります。
...
[class `App`](#app.py) の`present` メソッド内で、`rootViewController` の`presentViewController_animated_completion_` や、`UINavigationController` の`initWithRootViewController_` を Main Thread 処理する場面で使用しています。
...
NavigationBar に表示させるタイトルは、[`withModuleToSample.py` の Class `MainViewController`](#withModuleToSample.py) 内か、`MainViewController` インスタンス生成後に設定します。
...
これは、`RootNavigationController` クラスで定義した、インスタンスメソッド`navigationController_willShowViewController_animated_` を呼び出すためです。`RootNavigationController` は、`UINavigationController` のサブクラスなので、[`UINavigationControllerDelegate`](https://developer.apple.com/documentation/uikit/uinavigationcontrollerdelegate?language=objc) Protocol に準拠しています。そのため、[`navigationController:willShowViewController:animated:`](https://developer.apple.com/documentation/uikit/uinavigationcontrollerdelegate/navigationcontroller%28_:willshow:animated:%29?language=objc) を delegate 経由で呼び出すことができます。
...
`RootNavigationController` が乗った View が表示される直前に、`navigationController_willShowViewController_animated_` によって表示される ViewController の`navigationItem` 右肩に close ボタンを設置します。
...
close ボタン（[`UIBarButtonItem`](https://developer.apple.com/documentation/uikit/uibarbuttonitem?language=objc)）生成時（[`initWithBarButtonSystemItem_target_action_`](https://developer.apple.com/documentation/uikit/uibarbuttonitem/init%28barbuttonsystemitem:target:action:%29?language=objc)）にタップされた action を selector(`SEL`) で指定します。指定方法は、インスタンスメソッドの`closeButtonTapped_` を`SEL('closeButtonTapped:')` と、文字列化し`_` を`:` に書き換えるだけです。
...
close ボタン が押されたら、`self.dismissViewControllerAnimated_completion_(True, None)` が発動し、a-Shell から呼び出した全ての View を閉じます。
@[card](https://developer.apple.com/documentation/uikit/uiviewcontroller/dismiss%28animated:completion:%29?language=objc)

### `dealloc` で`loop` を止める

close ボタンが押されたことにより、全ての View が閉じられます。そのタイミングで、`dealloc` が呼ばれるので、走っている`loop` を`stop` し、`App.main_loop` の`loop.run_forever` が終わり`loop.close` することで event loop を終了させることができます。
...
a-Shell の`rootViewController` と、実装した View を繋ぎます。`RootNavigationController` を`self.viewController` の NavigationController とすることで、close ボタンを持った View を表示することができるようになります。
...
a-Shell から表示させる style で、型は`int` です。全画面表示指定（[`.fullScreen`](https://developer.apple.com/documentation/uikit/uimodalpresentationstyle/fullscreen?language=objc)）で、エラーが発生し a-Shell に戻れない事態を考慮して、デフォルトは[`.pageSheet`](https://developer.apple.com/documentation/uikit/uimodalpresentationstyle/pagesheet?language=objc) と、しています。
...
今回は使用していませんが、オブジェクトが持っている要素などを確認するのに使います。console に吐き出された結果から、Apple Developer Documentation で検索をしたり、オブジェクトの状態や親の Class を確認したりします。
...
module にまとめることにより、すっきりしたと思います。[`rootNavigationController`](#rootNavigationController.py) の時と同様に、View を表示する Life Cycle 系のメソッドは不要であれば、削除してしまって問題ありません。

実行すると、第 1 部と同じ View が表示されたと思います。次の第 3 部では、ここの部分にあたるファイルにコードを書いていきます。

## おまけの補足: Rubicon-ObjC での Function 定義方法
...
なお、わざわざ関数で、囲む必要はありません。開いたかたちでも可能だと思いますが:
...
module の管理として、見通しや利便性の観点でまとめています。
...
[NSStringFromClass](https://developer.apple.com/documentation/foundation/1395143-nsstringfromclass?language=objc) の場合は`Function` なので、自分で実装する部分があります。まず最初に注目する点は赤枠で囲っている、`Function` と、`Foundation` です。
...
Rubicon-ObjC で実装するにあたり、解説記事ブログも参考になりますが、[Apple Developer Documentation](https://developer.apple.com/documentation) で調べないと必要情報を得ることは難しい場面が多いです。

# おわりに

a-Shell アプリ自身と、Python 実装での ViewController 連携ができました。繋ぎ込みの部分は多少 hacky になってしまいましたが、各コードの役割を掴んでもらえていたら嬉しいです。

次回はこの module も活用し、オリジナルなアプリを開発します。Xcode は、Storyboard とコード両方を使う開発になりますが、a-Shell では全てコードベースで開発をします。
