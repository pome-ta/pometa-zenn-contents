...
Rubicon-ObjC ドキュメントの Asynchronous Programming with Rubicon  にて、iOS の[Integrating asyncio with iOS and UIApplication](https://rubicon-objc.readthedocs.io/en/stable/how-to/async.html#integrating-asyncio-with-ios-and-uiapplication) 解説があります。`loop.run_forever(lifecycle=iOSLifecycle())` と、`iOSLifecycle()` を使っていますが、a-Shell の場合は使いません。
